    ...
    while True:
        ...
        try:
            ...
            if isinstance(e, _expected_exceptions):
                if response is not None and response.status == 503:
                    ...
                    logger.warn("%s %s: %s. Waiting %d seconds due to server unavailability...",
                                method, _url, exception_msg, seconds_to_wait)
                ...
                if ok_to_retry:
                    ...
                    logger.warn("%s %s: %s. Waiting %d seconds before retry %d of %d... %s",
                                method, _url, exception_msg, delay, try_index + 1, max_retries, range_str)
                    time.sleep(delay)
                    try_index += 1
                    continue
            ...
            if not isinstance(e, exceptions.DXAPIError):
                logger.error("%s %s: %s", method, _url, exception_msg)
            ...
            if success and try_index > 0:
                logger.info("%s %s: Recovered after %d retries", method, _url, try_index)
